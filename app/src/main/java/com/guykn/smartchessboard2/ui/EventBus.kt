...
import com.guykn.smartchessboard2.ui.util.TimedEvent
...
class EventBus @Inject constructor() {
    sealed class ErrorEvent(duration: Long = 5*1000): TimedEvent(duration){
        ...
        class TooManyRequests(val timeForValidRequests: Long): ErrorEvent(-1)
        class SignInError: ErrorEvent()
        class NoLongerAuthorizedError: ErrorEvent(-1)
        class IllegalGameSelected: ErrorEvent(-1) // when a user tries to use the chessboard in a game that isn't rapid or classic time control, or a game with a rules variant.
